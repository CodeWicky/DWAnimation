...
- (void)viewDidLoad {
    ...
    self.desLb.center = CGPointMake(self.view.center.x, self.view.center.y + 150);
    
    [self initAnimation];
    [self configAnimationCallback];
    [self configTouchAction];
}
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if (self.ani.status == DWAnimationStatusPlaying ) {
        [self.ani suspend];
    } else if (self.ani.status == DWAnimationStatusSuspended) {
        [self.ani resume];
    } else {
        if (self.touchAction) {
            self.touchAction();
...
-(void)configAnimationCallback {
    self.ani.animationStart = ^(DWAnimation *ani) {
        NSLog(@"动画开始");
    };
    __weak typeof(self)weakSelf = self;
    self.ani.completion = ^(DWAnimation *ani) {
        NSLog(@"动画结束");
        [weakSelf stopTiming];
    };
}
...
-(UILabel *)timeLb {
    if (!_timeLb) {
        _timeLb = [[UILabel alloc] initWithFrame:CGRectMake(0, 84, 300, 50)];
    }
    return _timeLb;
}
